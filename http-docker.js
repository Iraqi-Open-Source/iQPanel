...
function activeAgent() {
  return agentClient.forServer(publicSettings().active_server_id || 'local');
}

async function handleDocker(request, response, pathname) {
  if (request.method === 'GET' && pathname === '/api/docker') {
    send(response, 200, await activeAgent().invoke('dockerStatus'));
    return true;
  }
  const actionMatch = pathname.match(/^\/api\/docker\/containers\/([^/]+)\/(start|stop|restart|remove)$/);
  if (actionMatch && request.method === 'POST') {
    const result = await activeAgent().invoke('dockerAction', actionMatch[1], actionMatch[2]);
  ...
  if (logsMatch && request.method === 'GET') {
    send(response, 200, await activeAgent().invoke('dockerLogs', logsMatch[1]));
    return true;
  }
  if (request.method === 'POST' && pathname === '/api/docker/compose') {
    ...
    const result = await require('./http-shared').siteAgent(site).invoke('dockerCompose', site, input.action || 'up');
    log('Docker compose', site.name, input.action || 'up');
    send(response, 200, result);
    return true;
  }
  if (request.method === 'POST' && pathname === '/api/docker/prune') {
    const result = await activeAgent().invoke('dockerPrune');
