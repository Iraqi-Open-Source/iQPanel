...
const { send, publicRoot, requestContext } = require('./http-shared');
const { handleAuth, sessionUser, sessionRecord, authorizeRequest } = require('./http-auth');
...
async function handleRequest(request, response) {
  ...
  await requestContext.run({ user, session }, async () => {
    if (await handleAuth(request, response, url.pathname)) return;
    if (url.pathname.startsWith('/api/')) {
      authorizeRequest(request, url.pathname);
      if (await handleWebhooks(request, response, url.pathname)) return;
      if (await handleUsers(request, response, url.pathname)) return;
      if (await handleAudit(request, response, url.pathname, url.searchParams)) return;
