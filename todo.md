...
- [x] Fix: 401/403 console errors — (1) sessions were in-memory only, so every panel restart logged everyone out and background calls flooded the console with 401s; sessions now persist in a `panel_sessions` sqlite table (login sessions and re-auth windows both survive restarts); (2) phase3a/phase3b fetched `/api/dashboard` and `/api/sites` at page load before login, producing 401 noise on the login screen — they now wait for the app to signal authentication (`iqpanel:authed` event); (3) a dismissed re-auth prompt no longer blocks future re-auth requests (stale pending-retry is cancelled)
- [x] Fix: terminal start produced a 403 in the console before showing the re-auth prompt — terminal starters (site Terminal tab + global Terminal view) now pre-check `/api/session` (`reauth_valid`) and open the password prompt proactively, so the session is created without any failed request; the global fetch re-auth interceptor remains as a backstop for other privileged actions
- [x] Fix: site Files tab showed an empty list with no explanation for freshly created sites — the `app/` directory is now auto-created on first list (returns an empty, usable directory), the empty state explains what to do ("Deploy your site, upload files, or use New file / New folder"), a missing-directory error renders a friendly hint, and the toolbar gained New file / New folder buttons; static assets now send `cache-control: no-cache` so browsers always pick up the latest panel JS; also widened the flaky rollback-job test timeout
